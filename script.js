...
if (heroVideo) {
  // Configure skipping the last N seconds of the video
  const SKIP_TAIL_SECONDS = 5;
  let startOffsetSeconds = 0; // updated per-device; desktop uses 20s below

  // Ensure the last N seconds are never played; when reaching end, jump back to startOffsetSeconds
  function setupSkipTail(secondsToSkip) {
    heroVideo.addEventListener("loadedmetadata", function () {
      if (!isFinite(heroVideo.duration) || heroVideo.duration <= secondsToSkip) return;
      const effectiveEndTime = Math.max(0, heroVideo.duration - secondsToSkip);
      heroVideo.addEventListener("timeupdate", function () {
        if (heroVideo.duration && heroVideo.currentTime >= effectiveEndTime) {
          heroVideo.currentTime = startOffsetSeconds;
  ...
  if (isIOS) {
    ...
    // Desktop starts from 20s and loops before the last 5s
  ...
  // Apply the skip-tail behavior for all devices
  setupSkipTail(SKIP_TAIL_SECONDS);
